...
- **Optional bevel** on the top edge
- **Optional round magnet recess** in the bottom (diameter and depth
  customizable) — the magnet is inserted from below
...
- Runs **entirely in the browser**: no backend, no build step, no network
  requests (three.js is vendored locally)
